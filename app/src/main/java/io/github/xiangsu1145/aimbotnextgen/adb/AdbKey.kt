...
class AdbKey(private val adbKeyStore: AdbKeyStore, private val name: String) {
    ...
    companion object {
        ...
        // PKCS#1 v1.5 signature padding followed by the SHA-1 DigestInfo prefix.
        // adbd verifies the AUTH token signature as RSASSA-PKCS1-v1_5(SHA-1, token),
        // which is exactly what this prefix encodes.
    ...
    constructor(context: Context) : this(PreferenceAdbKeyStore(context), "aimbot")

    private val encryptionKey: Key? = try {
        getOrCreateEncryptionKey()
    } catch (e: Throwable) {
        Log.w(TAG, "Failed to get AndroidKeyStore encryption key, private key will be stored unencrypted.", e)
        null
    }
    ...
    init {
        ...
        Log.d(TAG, "AdbKey ready, ${privateKey.modulus.toString(16).take(16)}...")
    }

    val adbPublicKey: ByteArray by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
    ...
    private fun getOrCreatePrivateKey(): RSAPrivateKey {
        ...
        if (stored != null) {
            // Encrypted form (matching Shizuku)
            decrypt(stored, aad)?.let { plaintext ->
                runCatching {
                    return KeyFactory.getInstance("RSA")
                        .generatePrivate(PKCS8EncodedKeySpec(plaintext)) as RSAPrivateKey
                }
            }
            // Plain PKCS#8 form (fallback when AndroidKeyStore is unavailable)
            runCatching {
                return KeyFactory.getInstance("RSA")
                    .generatePrivate(PKCS8EncodedKeySpec(stored)) as RSAPrivateKey
            }.onFailure { Log.w(TAG, "Stored adb key unreadable, regenerating.", it) }
        }
        ...
        val plaintext = generated.encoded
        val ciphertext = encrypt(plaintext, aad)
        // Same layout as Shizuku: AES-GCM ciphertext when AndroidKeyStore works,
        // otherwise fall back to plain PKCS#8 so pairing never becomes unusable.
        adbKeyStore.put(ciphertext ?: plaintext)
        return generated
    }
    ...
    /**
     * The SSLContext must always be created through the very same [TlsStrategy]
     * that later exports the keying material -- see [AdbConscrypt].
     */
    fun sslContext(strategy: TlsStrategy): SSLContext {
        return strategy.createSslContext(arrayOf(keyManager), arrayOf(trustManager), SecureRandom())
    }

    /** Convenience for regular ADB connections (no keying-material export involved). */
    val defaultSslContext: SSLContext by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
        (AdbConscrypt.strategies.firstOrNull() ?: object : TlsStrategy {
            override val name: String = "fallback"
            override fun createSslContext(
                keyManagers: Array<KeyManager>?,
                trustManagers: Array<TrustManager>?,
                random: SecureRandom?
            ) = SSLContext.getInstance("TLSv1.3").apply { init(keyManagers, trustManagers, random) }
...
class PreferenceAdbKeyStore(context: Context) : AdbKeyStore {

    private val preference: SharedPreferences =
        context.getSharedPreferences("adb_key", Context.MODE_PRIVATE)
...
private fun RSAPublicKey.adbEncoded(name: String): ByteArray {
    ...
    val nameBytes = " $name".toByteArray() + byteArrayOf(0)
