...
class reg_op_node {
  ...
  public:
    ...
    constexpr reg_op_node(op_reg_parms const p, uint16_t const n, uint16_t const t, bool s, bool g,
                          std::string_view const reg) noexcept
        : op_parms(p), op_name_offset(n), type_tag_offset(t), is_external(s), is_legacy(g), target_reg(reg)
    {
    }

    constexpr reg_op_node(op_reg_parms const p, uint16_t const n, uint16_t const t, bool s, bool g) noexcept
        : op_parms(p), op_name_offset(n), type_tag_offset(t), is_external(s), is_legacy(g), target_reg("core")
    {
    }

    /** @brief reg_op_node */
    constexpr reg_op_node() noexcept : reg_op_node(op_reg_parms{}, 0, 0, false, false, "core") {}
                      ...
                      std::string_view file_name) const
    {
        std::string_view const op_name = get_subview(op_name_strtab, op_name_offset);
        std::string_view const type_tag = get_subview(type_tag_strtab, type_tag_offset);
        hnnx::make_op_custom(op_name, type_tag, op_parms, is_legacy, file_name, target_reg);
    }
