...
#define IMPL_APPEND_REG_OP_ELEM_NO_TCM_FOLDING(I, FP, OP, TAG, IS_EXTERNAL, IS_LEGACY)                                 \
    ...
    constexpr ba_op<NC<(I)>::reg_op_count()> op_arr_container::chain<UniqTy<0>, NC<(I)>::reg_op_count()> =             \
            chain<UniqTy<0>, NC<(I - 1)>::reg_op_count()>.append(                                                      \
                    hnnx::reg_op_node{hnnx::GetParms::get<FP, I>(), StrUpd<(I)>::op_name_offset,                       \
                                      StrUpd<(I)>::type_tag_offset, IS_EXTERNAL, IS_LEGACY});

#define IMPL_APPEND_REG_OP_ELEM_NO_TCM_FOLDING_REG(I, FP, OP, TAG, IS_EXTERNAL, IS_LEGACY, REG)                        \
    IMPL_APPEND_REG_OP_ELEM_NO_TCM_FOLDING_BASE(I, FP, OP, TAG, IS_EXTERNAL)                                           \
    /** @brief Finally, append a new element to the Op registration table. */                                          \
    template <>                                                                                                        \
    template <>                                                                                                        \
    constexpr ba_op<NC<(I)>::reg_op_count()> op_arr_container::chain<UniqTy<0>, NC<(I)>::reg_op_count()> =             \
            chain<UniqTy<0>, NC<(I - 1)>::reg_op_count()>.append(                                                      \
                    hnnx::reg_op_node{hnnx::GetParms::get<FP, I>(), StrUpd<(I)>::op_name_offset,                       \
                                      StrUpd<(I)>::type_tag_offset, IS_EXTERNAL, IS_LEGACY, REG});
    ...
    constexpr ba_op<NC<(I)>::reg_op_count()> op_arr_container::chain<UniqTy<0>, NC<(I)>::reg_op_count()> =             \
            chain<UniqTy<0>, NC<(I - 1)>::reg_op_count()>.append(                                                      \
                    hnnx::reg_op_node{hnnx::GetParms::get<fold::ModifiedDerivedType<FP, LINE>::Modified, I>(),         \
                                      StrUpd<(I)>::op_name_offset, StrUpd<(I)>::type_tag_offset, false, IS_LEGACY});
...
#define APPEND_REG_OP_ELEM_NO_TCM_FOLDING(FP, OP, TAG, IS_EXTERNAL, IS_LEGACY)                                         \
    IMPL_APPEND_REG_OP_ELEM_NO_TCM_FOLDING(__COUNTER__, FP, OP, TAG, IS_EXTERNAL, IS_LEGACY)

#define APPEND_REG_OP_ELEM_NO_TCM_FOLDING_REG(FP, OP, TAG, IS_EXTERNAL, IS_LEGACY, REG)                                \
    IMPL_APPEND_REG_OP_ELEM_NO_TCM_FOLDING_REG(__COUNTER__, FP, OP, TAG, IS_EXTERNAL, IS_LEGACY, REG)
