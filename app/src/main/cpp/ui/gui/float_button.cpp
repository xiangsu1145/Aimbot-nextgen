...
// ── Design constants ─────────────────────────────────────────────────────────
// In dp, scaled by uiScale() at use: the button is a fixed-size UI element
// pinned to the surface, not part of the board's 1200-px-tall design space, so
// it does not scale with hudScale() — it takes the density-derived ratio
// instead, so it keeps its size across panels. 32 dp is the requested size;
// the inset leaves a chunky black square inside the white shell.
...
constexpr float kButtonInnerInset    = 8.0f;    // black square padding
constexpr float kButtonInnerRounding = 3.0f;

// A press that moves the finger this far (dp) before releasing is treated as a
// drag rather than a tap. Below this distance the press toggles the HUD; above
// it the press only repositions the button. 8 dp is "the finger obviously meant
// to drag" without making a tap require an unnaturally still finger.
...
/** The design constants above are dp; this turns them into surface px. */
float dp() { return uiScale(); }

/** Centres the button on the current surface, leaving g_positionInitialised set. */
void centreOnSurface(float dw, float dh) {
    const float size = kButtonSize * dp();
    g_btnX = (dw - size) * 0.5f;
    g_btnY = (dh - size) * 0.5f;
...
void clampToSurface(float dw, float dh) {
    const float size   = kButtonSize * dp();
    const float margin = kButtonMargin * dp();
    if (g_btnX < margin) g_btnX = margin;
    if (g_btnY < margin) g_btnY = margin;
    if (g_btnX + size > dw - margin) g_btnX = dw - margin - size;
    if (g_btnY + size > dh - margin) g_btnY = dh - margin - size;
}
...
void drawFloatButton() {
    ...
    const float size = kButtonSize * dp();
    g_buttonRect = HudRect{g_btnX, g_btnY, size, size};
    const ImVec2 mn(g_btnX, g_btnY);
    const ImVec2 mx(g_btnX + size, g_btnY + size);
    ...
    if (g_dragging && mouseDown) {
        ...
        const float dragPx = kDragThreshold * dp();
        if (!g_dragMoved && (std::fabsf(dx) > dragPx ||
                             std::fabsf(dy) > dragPx)) {
            g_dragMoved = true;
        }
        if (g_dragMoved) {
    ...
    // White rounded shell.
    dl->AddRectFilled(mn, mx, IM_COL32(255, 255, 255, 255), kButtonRounding * dp());

    // Black square inside — inset on every side so it sits centred with a
    // visible white border.
    const float inset = kButtonInnerInset * dp();
    dl->AddRectFilled(ImVec2(g_btnX + inset, g_btnY + inset),
                      ImVec2(g_btnX + size - inset, g_btnY + size - inset),
                      IM_COL32(0, 0, 0, 255), kButtonInnerRounding * dp());
#endif
}
